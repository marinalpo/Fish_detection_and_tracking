...
##        tf_util.restore(self.sess, ckpt.model_checkpoint_path)
    ...
    def track(self, unique_id, image, starting_box=None):
        with torch.no_grad():
            ...
            if starting_box is not None:
                ...
                #lstmState = [torch.autograd.Variable(torch.rand(1, 1, LSTM_SIZE).type(self.dtype)) for _ in range(4)]
                lstmState = [torch.rand(1, 1, LSTM_SIZE).cuda() for _ in range(4)]
                #lstmState = [torch.rand(1, 1, LSTM_SIZE) for _ in range(4)]
            ...
            batch_size = 1
            inputs = torch.cuda.FloatTensor([croppedInput0, croppedInput1]).reshape(2*batch_size, 3, CROP_SIZE, CROP_SIZE)
            #inputs = torch.FloatTensor([croppedInput0, croppedInput1]).reshape(2*batch_size, 3, CROP_SIZE, CROP_SIZE)
    ##        feed_dict = {
    ...
    ##                }
            ...
            outputBox = bb_util.from_crop_coordinate_system(rawOutput.squeeze().cpu().detach().numpy() / 10, pastBBoxPadded, 1, 1)
            ...
            if forwardCount > 0 and forwardCount % MAX_TRACK_LENGTH == 0:
                ...
                inputs = torch.cuda.FloatTensor(np.tile(croppedInput[np.newaxis,...], (2,1,1,1))).reshape(2*batch_size, 3, CROP_SIZE, CROP_SIZE)
        ...
        with torch.no_grad():
            ...
            for unique_id in unique_ids:
                ...
                for ss,state in enumerate(lstmState):
                    if type(lstmStates[ss]) is list:
                        lstmStates[ss] = state
                    else:
                        lstmStates[ss] = torch.cat([lstmStates[ss], state], dim=1)
            batch_size=int(len(images)/2)
            inputs = torch.cuda.FloatTensor(images).reshape(2*batch_size, 3, CROP_SIZE, CROP_SIZE)
    ##        for state in lstmStates:
    ##            lstmStateArrays.append(torch.tensor(np.array(state)).cuda())
    ...
    ##                }
            #rawOutput, s1, s2 = self.sess.run([self.outputs, self.state1, self.state2], feed_dict=feed_dict)
            rawOutput, s1, s2 = self.network(inputs=inputs, batch_size=batch_size, prevLstmState=lstmStates)
            outputBoxes = np.zeros((len(unique_ids), 4))
            for uu,unique_id in enumerate(unique_ids):
                lstmState, pastBBox, prevImage, originalFeatures, forwardCount = self.tracked_data[unique_id]
                lstmState = [s1[0][:,uu,:].reshape(1,1, LSTM_SIZE), s1[1][:,uu,:].reshape(1,1, LSTM_SIZE), s2[0][:,uu,:].reshape(1,1, LSTM_SIZE), s2[1][:,uu,:].reshape(1,1, LSTM_SIZE)]
                if forwardCount == 0:
                ...
                outputBox = bb_util.from_crop_coordinate_system(rawOutput[uu,:].squeeze().cpu().detach().numpy() / 10.0, pastBBoxPadded, 1, 1)

                if forwardCount > 0 and forwardCount % MAX_TRACK_LENGTH == 0:
                    croppedInput, _ = im_util.get_cropped_input(image, outputBox, CROP_PAD, CROP_SIZE)
                    #input = np.tile(croppedInput[np.newaxis,...], (2,1,1,1))
                    inputs = torch.cuda.FloatTensor(np.tile(croppedInput[np.newaxis,...], (2,1,1,1))).reshape(2, 3, CROP_SIZE, CROP_SIZE)
##                    feed_dict = {
##                            self.imagePlaceholder : input,
##                            self.prevLstmState : originalFeatures,
##                            self.batch_size : 1,
##                            }
                    ...
                    _ , s1_new, s2_new = self.network(inputs=inputs, batch_size=1, prevLstmState=originalFeatures)
