...
##            param.requiresGrad = False
        ...
        self.pool_shallow = torch.nn.AdaptiveAvgPool2d(output_size=(12, 24))
        self.pool_intermediate = torch.nn.AdaptiveAvgPool2d(output_size=(6, 6))
        self.pool_deep = torch.nn.AdaptiveAvgPool2d(output_size=(3, 3))
        
    def forward(self, input, batch_size, num_unrolls):

        # Normalizing the input        
        for i, m, s in zip(input, IMAGENET_MEAN, IMAGENET_STD):
            i.sub_(m).div_(s)
        # Shallow layers
        shallow = self.shallow(input) #[N, 3, K, L]->[N, 64, K/2, L/2]
...
##                                 pool_out.reshape(1*2,1,2048)], dim=0)
        ...
        skip_concat = torch.cat([shallow_out.reshape(batch_size*2,2048,3,3),
                                intermediate_out.reshape(batch_size*2,2048,3,3),
                                deep_out.reshape(batch_size*2,2048,3,3)], dim=1)
...
class torch_net(nn.Module):
    def __init__(self):
        ...
        self.lstm1 = nn.LSTM(4*LSTM_SIZE, LSTM_SIZE)
        self.lstm2 = nn.LSTM(5*LSTM_SIZE, LSTM_SIZE)
