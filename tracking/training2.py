...
def train_model(model, conv, dataloaders, criterion, optimizer, num_epochs=1):
    since = time.time()
            ...
            for num_batch, (inputs_sequence, labels_sequence) in enumerate(zip(inputs_sequences, labels_sequences)):
                ...
                for i, _ in enumerate(labels_input_vec):
                        ...
                        with torch.set_grad_enabled(phase == 'train'):
                            conv_out = conv(inputs=inputs,batch_size=1)
...
# Import the model
conv = network_resnet2.resnet_conv_layers()
conv.eval()
model_ft = network_resnet2.torch_net()
...
# Train and evaluate
model_ft, hist = train_model(model_ft, conv, dataloaders_dict, criterion, optimizer_ft, num_epochs=NUM_EPOCHS)
